...
WinBanner::WinBanner() {
	...
	scale = glm::vec3(.005f, .005f, .005f);
	rotation = glm::vec4(1.0, 0, 0, 0);
}

void WinBanner::Render(glm::mat4 view, glm::mat4 proj, glm::vec3 centerPos) {
...
glm::vec3 WinBanner::GetPos() {
	return position;
}

glm::vec3 WinBanner::GetScale() {
	return scale;
}

glm::vec4 WinBanner::GetRot() {
	return rotation;
}

Model * WinBanner::GetModel() {
	return &model;
}